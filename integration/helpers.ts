...
/**
 * Set up admin access: call setAdmins then initializeAuthority.
 * Requires the upgrade authority keypair (used as payer for setAdmins).
 */
export async function registerAuthority(
  program: Program<ScheduledTransfer>,
  upgradeAuthority: Keypair,
  admin: Keypair,
  authority: PublicKey,
): Promise<void> {
  // Set admin list (upgrade authority sets itself or admin as admin)
  const upgradeProgram = createProgram(
  ...
  // Admin registers the authority
  const adminProgram = createProgram(
    program.provider.connection,
    admin,
  );
...
/**
 * Load the upgrade authority keypair generated by setup-validator.ts.
 */
export function loadUpgradeAuthority(): Keypair {
  const fixturePath = path.join(
    import.meta.dirname!,
    "fixtures",
