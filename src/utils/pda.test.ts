...
import { findPaymentSchedulePda, findPaymentRecordPda } from "./pda";
...
describe("findPaymentSchedulePda", () => {
  it("returns a PublicKey and bump", () => {
    const [pda, bump] = findPaymentSchedulePda(authority);
  ...
  it("is deterministic for the same authority", () => {
    const [pda1] = findPaymentSchedulePda(authority);
    const [pda2] = findPaymentSchedulePda(authority);
  ...
  it("differs for different authorities", () => {
    ...
    const [pda1] = findPaymentSchedulePda(authority);
    const [pda2] = findPaymentSchedulePda(other);
